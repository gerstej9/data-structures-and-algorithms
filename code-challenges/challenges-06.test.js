...
const templatingWithMustache = () => {
  ...
  let characterHtml = Mustache.render(template, character);
  templateArr.push(characterHtml);
  });
  return templateArr;
};
...
const getCourseKeys = (obj) => {
  const courseKeys = []
  for(let keyCourse in courseInfo){
    courseKeys.push(keyCourse)
  }
  return courseKeys;
};
...
const getHouses = (arr) => {
  let houses = [];
  for(let valueHouse of characters){
    houses.push(valueHouse.house);
  }
  return houses;
};
...
const houseSize = (arr) => {
  let sizes = [];
  arr.forEach(char => {
  sizes.push({house:char.house, members: 0})
  });
  arr.forEach(char =>{
    for(let i = 0; i < sizes.length; i++){
      if(char.house === sizes[i].house){
        sizes[i].members += 1;
        if(char.spouse){
          sizes[i].members +=1;
        }
        if(char.children){
          sizes[i].members += char.children.length;
        }
      }
    }
  })
...
const houseSurvivors = (arr) => {
  const survivors = [];
  arr.forEach(char => {
    survivors.push({house:char.house, members: 0})
  });
  arr.forEach(char =>{
    for(let i = 0; i < survivors.length; i++){
      if(char.house === survivors[i].house){
        survivors[i].members += 1;
        if(char.spouse && !deceasedSpouses.includes(char.spouse)){
          survivors[i].members +=1;
        }
        if(char.children){
          survivors[i].members += char.children.length;
        }
      }
    }
  })
