...
const filterStringsWithVowels = (arr) => {
  const vowelArr = arr.filter(value => {
    const regex = /(a|e|i|o|u)/;
    const vowelWord = regex.test(value);
    return vowelWord; 
  });
  return vowelArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  const goodArr = arr.filter(value =>{
    if (forbiddenValues.includes(value) !== true){
      return value;
    }
  })
  return goodArr
...
const getStatName = (arr, minBaseStat) => {
  const baseStatArr = arr.filter(stat => stat.baseStat > minBaseStat);
  return baseStatArr.map(stats => stats.stat.name)
};
...
const evenOddNumericValues = (arr) => {
  const medArr = arr.filter(int => typeof(int) === 'number');
  return medArr.map(int =>{
    if(int %2 === 0){
      return 'even'
    }else{
      return 'odd'
