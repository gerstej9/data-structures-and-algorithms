...
const createServer = () => {
  ...
  app.get('/aboutme', function(req,res){
    res.send('I am an awesome, super coold dude')
  });
  app.get('/favoritefoods', function(req,res){
    res.send(['carrots', 'ice cream', 'ramen', 'cookies', 'bulgogi'])
  });
...
const number =  input[i].filter(digit => digit === target);
countTarget += number.length;
}
 return countTarget;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  let newArr = []
  input.forEach(arr =>{
    let miniArr = arr.filter(value => typeof(value) === 'number');
    let min5Arr = miniArr.filter(a => a % 5 === 0);
    let powerArr = min5Arr.map(x => Math.pow(2, x));
    newArr.push(powerArr);
  })
  return newArr;
};
...
let findMaleAndFemale = (data) => {
  let genderArr = data.filter(person => person.gender === 'female' || person.gender === 'male');
  const reducer = (a,b) => a+ ' and '+ b.name;
  let str = genderArr.reduce(reducer, '');
...
let findShortest = (data) => {
  ...
  const person = data.reduce(reducer, null)
  return person.name
};
